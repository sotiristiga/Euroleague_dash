import urllib.requestAdd commentMore actions
...
with f1:
    search_team_team1=st.selectbox("Choose  Team:",All_Seasons['Team'].reset_index().sort_values('Team')['Team'].unique())
with f2:
    search_team_season_team1 = st.selectbox("Season:",['2016-2017', '2017-2018', '2018-2019', '2019-2020', '2020-2021','2021-2022', '2022-2023', '2023-2024','2024-2025','All'],index=8)
with f3:
    search_team_phase_team1 = st.selectbox("Phase:",['Regular Season', 'Play In','Play offs', 'Final Four','All'],index=4) 
with f4:
    search_team_round_team1 = st.selectbox("Round:",['First Round', 'Second Round','PI 1', 'PI 2', 'PO 1', 'PO 2', 'PO 3', 'PO 4','PO 5', 'Semi Final', 'Third Place', 'Final', 'All'],index=12)
with f5:
...
if "All" in search_team_season_team1:
    ...
    period_points1 = period_points.loc[period_points['Season'].isin(search_team_season_team1)]Add commentMore actions
...
if "All" in search_team_phase_team1:
    ...
    select_allstats_in_a_game2 = select_allstats_in_a_game2.loc[
        select_allstats_in_a_game2['Phase'].isin(search_team_phase_team1)]Add commentMore actions
...
def compute_player_stats_by_team_against(dataset,Team):
    ...
    computestatsfinal = pd.merge(player_extra, computestatsfinal)Add commentMore actions
    computestatsfinal=computestatsfinal.set_index('Player').round(1)
...
Add commentMore actions
    ...
    interactive_table(pd.concat([parts, playoff, finalfour, title]).set_index('Euroleague'),paging=False, height=900, width=2000, showIndex=True,Add commentMore actions
...
def compute_team_stats_against_each_team(dataset_stats1,dataset_stats2,dataset_periods):
    teamstats=dataset_stats1.loc[dataset_stats1.Team==search_team_team1]Add commentMore actions
    oppstats=dataset_stats2.loc[dataset_stats2.Against==search_team_team1]
  finalstats=teamstats.groupby(['idseason','Against'])[['PTS','F2M',Add commentMore actions
                                  'F2A', 'F3M', 'F3A', 'FTM', 'FTA', 'OR',
                                  'DR', 'TR', 'AS', 'ST', 'TO', 'BLK', 'BLKR','PF', 'RF',
                                  'PIR','Possesions']].sum().reset_index().groupby('Against')[['PTS','F2M',
                                  'F2A', 'F3M', 'F3A', 'FTM', 'FTA', 'OR',
...
interactive_table(compute_team_stats_against_each_team(All_Seasons1,All_Seasons2,period_points1).set_index('Against'),
                      paging=False, height=1000, width=2000, showIndex=True,Add commentMore actions
                      classes="display order-column nowrap table_with_monospace_font", searching=False,fixedColumns=True, select=True, info=False, scrollCollapse=True,Add commentMore actions
                      scrollX=True, scrollY=1000, fixedHeader=True, scroller=True, filter='bottom',
                      columnDefs=[{"className": "dt-center", "targets": "_all"}])
...
select_allstats_in_a_game1['SHS Before'] = (
    ...
    select_allstats_in_a_game1['index'])).round(1)Add commentMore actions
select_allstats_in_a_game1['SHC Before'] = (
            (select_allstats_in_a_game1['SHC'].cumsum() - select_allstats_in_a_game1['SHC']) / (
    select_allstats_in_a_game1['index'])).round(1)
select_allstats_in_a_game1['PTS Before'] = (Add commentMore actions
...
select_allstats_in_a_game1['TO conc Before'] = (
    ...
    select_allstats_in_a_game1['index'])).round(1)
select_allstats_in_a_game1['PF conc Before'] = (Add commentMore actions
...
select_allstats_in_a_game1.drop(['index','Team'],axis=1,inplace=True)Add commentMore actions
team_ranking_stat = st.selectbox("Select Stat:", ["All",'PTS', '2P', '3P', 'FT', 'OR', 'DR',Add commentMore actions
                                                       'TR', 'AS', 'STL', 'TO', 'BLK', 'PF', 'PIR'], index=0)
if team_ranking_stat=="All":Add commentMore actions
    interactive_table(select_allstats_in_a_game1.set_index('Against').sort_values('Fixture', ascending=True),paging=False, height=900, width=2000, showIndex=True,
                      classes="display order-column nowrap table_with_monospace_font", searching=False,fixedColumns=True, select=True, info=False, scrollCollapse=True,Add commentMore actions
                      scrollX=True, scrollY=1000, fixedHeader=True, scroller=True, filter='bottom',
                      columnDefs=[{"className": "dt-center", "targets": "_all"}])
else:
    regex1 = "Against|Season|Phase|Round|Fixture|HA|results" + "|" + team_ranking_stat
  interactive_table(Add commentMore actions
        select_allstats_in_a_game1.filter(regex=regex1).set_index('Against').sort_values('Fixture', ascending=True),paging=False, height=900, width=2000, showIndex=True,Add commentMore actions
        classes="display order-column nowrap table_with_monospace_font", searching=False,fixedColumns=True, select=True, info=False, scrollCollapse=True,Add commentMore actions
        scrollX=True, scrollY=1000, fixedHeader=True, scroller=True, filter='bottom',
        columnDefs=[{"className": "dt-center", "targets": "_all"}])
